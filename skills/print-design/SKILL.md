...
* **一次情報にない事実の推測・補完の絶対禁止（事実の厳格性）**:
  - イベントレギュレーション（カメラ制限、手荷物預かりの有無、飲食ルール等）やメンバー名・固有名詞について、元情報（公式サイト・告知文）に書かれていない内容を「一般的だろう」と勝手に推測して明示してはなりません。
  - 「撮可TIMEあり」とだけ書かれている場合に「一眼レフ可」等と断定してはならず、記載がない場合は「詳細は当日会場アナウンスをご確認ください」等に留めます。また、ロッカーの有無やマスク義務などの基本注意事項の真逆の記述（例: 「ロッカーなし」を「手荷物置場あり」とする等）を防ぐため、公式全文の精査を徹底します。
