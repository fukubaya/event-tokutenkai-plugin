...
* **開催場所・前提情報の配置設計**:
  - 最重要の前提情報である「開催場所・会場アクセス」は、下部の注意事項欄ではなく、タイムテーブルの最上部（10:30などの最初の項目の上）に1列分の独立枠として配置します。左側に時刻ピルと同じ幅（26mm）の「📍 開催場所」バッジを設けることで、タイムライン全体の縦の整列軸を乱さずに自然に配置できます。
* **4Kディスプレイ・印刷品質でのプレビュー画像生成**:
  - 生成されたPDFを画像化して目視確認する際は、文字潰れを防ぐため `qlmanage -t -s 3508 -o <dir> <pdf-file>` 等で A4 300dpi相当（2480×3508px）以上の高精細PNGを出力し、4K環境でも細かな文字・注釈までクッキリ確認できるようにします。
* **会場フロアマップのベクターSVG統合**:
  - ステージ、優先観覧エリア、女性専用エリア、入場導線等の会場図が提供されている場合は、D2 CLIやベクターSVG（`floor_map.svg`）として自作・再構成します。開催場所カードの右側にコンパクトにインライン配置することで、印刷時にも文字が一切潰れず、場所情報とエリア区分が一目で把握できます。
