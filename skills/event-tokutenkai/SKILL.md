...
* **会場フロアマップのD2描画原則と「入場案内枠」統合（HTML div構築の絶対禁止）**:
  - 会場フロア図（ステージ、優先観覧エリア、女性専用エリア、カメラエリア、新規エリア、フリーエリア等）を **HTML/CSS（`div`タグの入れ子）で無理やり組むことは禁止** します。優先エリア内のサブエリア（新規、女性、カメラ）やグルショ最前枠などの包含関係が崩れて不正確になるためです。
  - 必ず D2言語スクリプト（`floormap1.d2`, `floormap2.d2`）として記述し、`flyer.py d2` または `flyer.py all` の自動コンパイル機能を用いて高精度・クリーンなベクターSVG（ライトテーマ `--theme 0`）を生成・配置してください。
...
* **特典券付与レギュレーションの厳格な記述原則（選択制 vs 複数枚の識別）**:
  - 対象商品購入時の特典券付与について、「1枚購入で①②③から1枚選択」なのか「①②③各1枚（計3枚）もらえる」のか、文意を極めて慎重に読み取り、誤った数量・条件を記載してはなりません。重大な現場トラブルの原因となります。
