# Event & Tokutenkai Flyer Plugin for Antigravity CLI (v0.2.1)
...
さらに **v0.2.0** で導入されたスマートフォン・SNS（X/Twitter等）特化の**「SNS告知用 4枚カルーセル戦略」**に加え、**v0.2.1** では「過去生成物・他イベントの参照絶対禁止（プラグイン自己完結性の担保）」「`flyer.py init` による自己完結プロジェクト初期化」「公式コンポーネント集（`references/components.md`）」「公式告知の事実厳守」「撮影ルールの条件付き表現徹底」「カルーセルハイライトリボンの文字数最適化」を体系化し、外部プロジェクトや他環境でも完全に独立して動作する設計を確立しました。
...
├── plugin.json               # プラグインマニフェスト (v0.2.1)
├── README.md                 # プラグイン説明書
├── rules/
...
└── skills/
    └── event-tokutenkai/
        ...
        ├── references/
        │   └── components.md # 公式デザインコンポーネント・パターン集（事前予約4STEP等）
        ├── assets/
...
### 0. 新規プロジェクトの自己完結初期化 (`init`) 【v0.2.2 新機能】

プラグイン公式テンプレートから、新規イベントディレクトリに必要なファイル（HTML, CSS, 背景ベクターSVG, QRコード, アセットリンク）を一発で初期配備します。**他イベントのファイルを参照・コピーする必要は一切ありません**。

```bash
# SNS用4枚カルーセルプロジェクトの初期化（公式QRコードも自動生成）
uv run python skills/event-tokutenkai/scripts/flyer.py init events/20261015-release --type carousel --url "https://example.com/event"

# A4単枚印刷フライヤープロジェクトの初期化
uv run python skills/event-tokutenkai/scripts/flyer.py init events/20261015-release --type flyer --url "https://example.com/event"
```
...
* **v0.2.1**:
  * **過去生成物・他イベントの参照絶対禁止（プラグイン自己完結性の担保）**: 過去のイベント成果物（`events/` 配下等）を参照することを厳禁とし、過去データの古い文脈による未発表情報の捏造やハルシネーション（架空のユニット構成等）を根本から防止。
  * **公式告知の事実厳守（推測・未発表情報の完全排除）**: イベント告知本文に明記されていないユニット構成や所属メンバー内訳を、CD収録曲や外部情報から推測して補完・掲載することを厳禁とし、公式発表されているレーン分け（グループ分け）をメインに据える原則を策定。
  * **撮影・録音ルールの条件付き表現徹底（指示・指定時例外の明記）**: 「メンバー・スタッフからの指定がない場合での撮影・録音禁止」という告知文に対し、「写真撮影全面禁止」「撮可なし」と過剰に断定せず、「指定時以外の撮影禁止（指示・指定時を除く）」として公式の文脈（撮可タイムの可能性）を忠実に反映。
  * **プロジェクト自己完結初期化コマンド（`flyer.py init`）の追加**: テンプレートから新規イベントに必要なHTML、CSS、SVG、QRコード、アセットを一発で初期配備し、他イベントのコピーや参照を一切不要化。
  * **公式デザインコンポーネント集（`references/components.md`）の策定**: 事前予約（前金）4ステップフロー、2部制タイムライン、イラストグリッド、マナーストリップ、メンバーチップ等の標準スニペット集を公式同梱。
  * **カルーセルハイライトリボンの文字数最適化（枠外はみ出し防止）**: カルーセル1枚目の3カラムハイライトリボンにおいて、文字あふれ（オーバーフロー）を防ぐテキスト簡潔化ガイドラインおよびCSSスタイル（`min-width: 0;`、パディング・フォントサイズ調整）をテンプレートに反映。

* **v0.2.0**:
  * **SNS告知用 4枚カルーセル戦略の導入**: スマホ・X（Twitter）縦長画面に特化した4枚組カルーセルHTMLテンプレート（`templates/carousel/`）および一気通貫パイプライン（`flyer.py carousel`）の追加。
  * **高信頼スライス＆マージビルド**: Chromiumメモリ制限によるクラッシュを回避し、安定して4ページのベクターPDFおよび4枚の300dpi高精細PNGを出力。

* **v0.1.0**:
  * 初版リリース。A4単枚フライヤー組版、Vivliostyle / WeasyPrint ビルド、D2フロアマップ連携、ベクターSVG QRコード生成。
